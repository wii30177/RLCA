...
# -*- coding: utf-8 -*-
"""
PPO for Hopper-v5 — Factorized-RLCA partial config t|lf (11+11² = 132 logits)

基於 ppo_hopper-v5.py（連續 Gaussian baseline）的最小修改版，只動四處：
  (1) DiscretizeHopperAction wrapper（MultiDiscrete([11,121]) + LUT）
      dim0 = thigh 獨立索引；dim1 = (leg, foot) 耦合索引（11 進位 2 位數）
...
class DiscretizeHopperAction(gym.ActionWrapper):
    ...
    dim0 = thigh 的獨立離散索引；dim1 = (leg, foot) 的耦合索引（視為 11 進位 2 位數）。
    ...
    def action(self, act):
        # act: array-like of shape (2,) = (thigh_idx, leg_foot_idx)
        thigh_idx = int(act[0])
        lf_idx    = int(act[1])
        leg_idx   = lf_idx // self.bins   # 11 進位高位 → leg
        foot_idx  = lf_idx % self.bins    # 11 進位低位 → foot
        return np.array(
...
class Actor(nn.Module):
    """
    Factorized categorical policy (partial, t|lf): thigh 一顆獨立 head（11 logits），
    (leg, foot) 一顆耦合 head（121 logits）→ 11+121 = 132。
...
class PPO:
    ...
    def save_model(self, env_name, suffix="", actor_path=None, critic_path=None):
        ...
        if actor_path is None:
            actor_path  = f"preTrained/ppo_actor_logits132_t_lf_{env_name}_{suffix}"
        if critic_path is None:
            critic_path = f"preTrained/ppo_critic_logits132_t_lf_{env_name}_{suffix}"
...
def train():
    ...
    wandb.init(
        project="ppo",
        name=f"{env_name}_logits132_t_lf_seed{random_seed}",
        config={
            "env":             env_name,
            "variant":         "logits132_t_lf",
            "bins":            bins,
...
if __name__ == '__main__':
    ...
    # test("preTrained/ppo_actor_logits132_t_lf_Hopper-v5_stepXXX_epXXX.pth",
    #      "preTrained/ppo_critic_logits132_t_lf_Hopper-v5_stepXXX_epXXX.pth",
    #      render=True)
