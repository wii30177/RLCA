...
# -*- coding: utf-8 -*-
"""
PPO (Proximal Policy Optimization) for Hopper-v5
...
# ─── Actor: Gaussian policy for continuous action spaces ──────────────────────
class Actor(nn.Module):
    """
    Stochastic policy network that outputs a Gaussian distribution over actions.

    The mean is state-dependent (from the network); log_std is a learnable
    parameter shared across all states (standard PPO practice — simpler and
    often works as well as state-dependent std).

    At test time, use mean directly (deterministic greedy policy).
    """
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        # State-independent log_std: one learnable scalar per action dimension
        ...
        nn.init.orthogonal_(self.mean_head.weight, gain=0.01)  # small final-layer gain
    ...
    def evaluate_actions(self, x, actions):
        """
        Compute log π_θ(a|s) and entropy for given (state, action) pairs.
...
class PPO:
    ...
    def save_model(self, env_name, suffix="", actor_path=None, critic_path=None):
        ...
        if actor_path is None:
            actor_path  = f"preTrained/ppo_actor_{env_name}_{suffix}"
        if critic_path is None:
            critic_path = f"preTrained/ppo_critic_{env_name}_{suffix}"
...
def test(actor_path, critic_path, hidden_size=256, n_episodes=20, render=False):
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        while True:
            # Deterministic evaluation: use the mean of the Gaussian policy
            with torch.no_grad():
                mean, _ = model.actor(
                    torch.FloatTensor(np.array(state)).unsqueeze(0)
                )
...
if __name__ == '__main__':
    ...
    test("preTrained/ppo_actor_Hopper-v5_step351272_ep1468.pth",
         "preTrained/ppo_critic_Hopper-v5_step351272_ep1468.pth",
         render=True)
