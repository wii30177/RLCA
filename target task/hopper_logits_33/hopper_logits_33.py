...
# -*- coding: utf-8 -*-
"""
PPO for Hopper-v5 — Factorized-RLCA decoupled config (11+11+11 = 33 logits)

基於 ppo_hopper-v5.py（連續 Gaussian baseline）的最小修改版，只動四處：
  (1) DiscretizeHopperAction wrapper（MultiDiscrete([11,11,11]) + LUT）
  (2) Actor 輸出層：Gaussian mean/log_std → 3 顆獨立 Categorical head（thigh|leg|foot）
...
class DiscretizeHopperAction(gym.ActionWrapper):
    """
    Hopper-v5 的 3-D 連續動作 → MultiDiscrete([bins, bins, bins])（decoupled, 33 logits）。
    對外動作為長度 3 的整數向量 (i, j, k)，分別是 thigh / leg / foot 的離散索引。
    每一維各自離散到 bins 個值（預設 [-1.0, -0.8, ..., 1.0]），索引經查表 (LUT) 轉成實際 torque。
    """
    def __init__(self, env, bins=11, low=-1.0, high=1.0):
        ...
        self.action_space = gym.spaces.MultiDiscrete([bins, bins, bins])

    def action(self, act):
        # act: array-like of shape (3,) = (thigh_idx, leg_idx, foot_idx)
...
class Actor(nn.Module):
    """
    Factorized categorical policy (decoupled): one independent head per joint
    (thigh | leg | foot), each outputting `bins` logits → 11+11+11 = 33.
    ...
    def __init__(self, hidden_size, num_inputs, action_space):
        super().__init__()
        nvec = [int(n) for n in action_space.nvec]  # [11, 11, 11]
...
class PPO:
    ...
    def save_model(self, env_name, suffix="", actor_path=None, critic_path=None):
        ...
        if actor_path is None:
            actor_path  = f"preTrained/ppo_actor_logits33_{env_name}_{suffix}"
        if critic_path is None:
            critic_path = f"preTrained/ppo_critic_logits33_{env_name}_{suffix}"
...
def train():
    ...
    wandb.init(
        project="ppo",
        name=f"{env_name}_logits33_seed{random_seed}",
        config={
            "env":             env_name,
            "variant":         "logits33_decoupled",
            "bins":            bins,
            "n_logits":        bins * 3,   # 11+11+11 = 33
            "seed":            random_seed,
...
if __name__ == '__main__':
    ...
    # test("preTrained/ppo_actor_logits33_Hopper-v5_stepXXX_epXXX.pth",
    #      "preTrained/ppo_critic_logits33_Hopper-v5_stepXXX_epXXX.pth",
    #      render=True)
