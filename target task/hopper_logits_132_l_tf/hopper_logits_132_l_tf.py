...
# -*- coding: utf-8 -*-
"""
PPO for Hopper-v5 — Factorized-RLCA partial config l|tf (11+11² = 132 logits)

基於 ppo_hopper-v5.py（連續 Gaussian baseline）的最小修改版，只動四處：
  (1) DiscretizeHopperAction wrapper（MultiDiscrete([11,121]) + LUT）
      dim0 = leg 獨立索引；dim1 = (thigh, foot) 耦合索引（11 進位 2 位數）
  (2) Actor 輸出層：Gaussian mean/log_std → 2 顆 Categorical head（11 + 121）
  (3) test()：取 mean → 各 head argmax
...
class DiscretizeHopperAction(gym.ActionWrapper):
    """
    Hopper-v5 的 3-D 連續動作 → MultiDiscrete([bins, bins**2])（partial, 11+121=132 logits）。
    dim0 = leg 的獨立離散索引；dim1 = (thigh, foot) 的耦合索引（視為 11 進位 2 位數）。
    ...
    def __init__(self, env, bins=11, low=-1.0, high=1.0):
        ...
        self.action_space = gym.spaces.MultiDiscrete([bins, bins ** 2])

    def action(self, act):
        # act: array-like of shape (2,) = (leg_idx, thigh_foot_idx)
        leg_idx   = int(act[0])
        tf_idx    = int(act[1])
        thigh_idx = tf_idx // self.bins   # 11 進位高位 → thigh
        foot_idx  = tf_idx % self.bins    # 11 進位低位 → foot
        return np.array(
...
class Actor(nn.Module):
    """
    Factorized categorical policy (partial, l|tf): leg 一顆獨立 head（11 logits），
    (thigh, foot) 一顆耦合 head（121 logits）→ 11+121 = 132。

    Joint action is assumed conditionally independent across heads:
    ...
    def __init__(self, hidden_size, num_inputs, action_space):
        super().__init__()
        nvec = [int(n) for n in action_space.nvec]  # [11, 121]
...
class PPO:
    ...
    def save_model(self, env_name, suffix="", actor_path=None, critic_path=None):
        ...
        if actor_path is None:
            actor_path  = f"preTrained/ppo_actor_logits132_l_tf_{env_name}_{suffix}"
        if critic_path is None:
            critic_path = f"preTrained/ppo_critic_logits132_l_tf_{env_name}_{suffix}"
...
def train():
    ...
    wandb.init(
        project="ppo",
        name=f"{env_name}_logits132_l_tf_seed{random_seed}",
        config={
            "env":             env_name,
            "variant":         "logits132_l_tf",
            "bins":            bins,
            "n_logits":        bins + bins ** 2,   # 11+121 = 132
            "seed":            random_seed,
...
if __name__ == '__main__':
    ...
    # test("preTrained/ppo_actor_logits132_l_tf_Hopper-v5_stepXXX_epXXX.pth",
    #      "preTrained/ppo_critic_logits132_l_tf_Hopper-v5_stepXXX_epXXX.pth",
    #      render=True)
