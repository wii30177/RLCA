...
# -*- coding: utf-8 -*-
"""
PPO for LunarLanderContinuous-v3 — Factorized-RLCA joint config (11² = 121 logits)

基於 ppo_lunar-v3.py（連續 Gaussian baseline）的最小修改版，只動四處：
  (1) DiscretizeLunarLanderAction wrapper（Discrete(121) + LUT，11 進位 2 位數拆解）
  (2) Actor 輸出層：Gaussian mean/log_std → 單顆 121-logits Categorical head
  (3) test()：取 mean → argmax（並改為 render 參數控制視窗，預設關閉）
...
class DiscretizeLunarLanderAction(gym.ActionWrapper):
    """
    LunarLanderContinuous 的 2-D 連續動作 → Discrete(bins**2)（joint, 121 logits）。
    對外動作為 0..120 的整數，視為 11 進位 2 位數：(main, lateral)。
    每個 engine 離散到 bins 個值（預設 [-1.0, -0.8, ..., 1.0]），索引經查表 (LUT) 轉成實際推力。
    ...
    def __init__(self, env, bins=11, low=-1.0, high=1.0):
        ...
        self.action_space = gym.spaces.Discrete(bins ** 2)

    def action(self, act):
        # act: 純量或 shape (1,) 的整數，11 進位 2 位數 → (main, lateral)
        index       = int(np.asarray(act).reshape(-1)[0])
        main_idx    = index // self.bins   # 高位 → 主引擎
        lateral_idx = index % self.bins    # 低位 → 副引擎
        return np.array(
            [self.table[main_idx], self.table[lateral_idx]],
            dtype=np.float32,
        )
...
class Actor(nn.Module):
    """
    Joint categorical policy: 單顆 head 輸出 11² = 121 logits，
    一次選定 (main, lateral) 的完整組合（flatten 的 joint 空間，對照組）。
...
class PPO:
    ...
    def save_model(self, env_name, suffix="", actor_path=None, critic_path=None):
        ...
        if actor_path is None:
            actor_path  = f"preTrained/ppo_actor_logits121_{env_name}_{suffix}"
        if critic_path is None:
            critic_path = f"preTrained/ppo_critic_logits121_{env_name}_{suffix}"
...
def train():
    ...
    wandb.init(
        project="ppo",
        name=f"{env_name}_logits121_seed{random_seed}",
        config={
            "env":             env_name,
            "variant":         "logits121_joint",
            "bins":            bins,
            "n_logits":        bins ** 2,   # 11² = 121
            "seed":            random_seed,
...
if __name__ == '__main__':
    ...
    # test("preTrained/ppo_actor_logits121_LunarLanderContinuous-v3_stepXXX_epXXX.pth",
    #      "preTrained/ppo_critic_logits121_LunarLanderContinuous-v3_stepXXX_epXXX.pth",
    #      render=True)
