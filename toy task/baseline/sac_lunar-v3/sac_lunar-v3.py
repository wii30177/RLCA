...
# -*- coding: utf-8 -*-
"""
SAC (Soft Actor-Critic) for LunarLanderContinuous-v3
...
def train():
    # ── Hyperparameters ──────────────────────────────────────────────────────
    total_steps      = 500_000     # total env steps (LunarLander converges faster than Hopper)
    ...
    replay_size      = 500_000     # replay buffer capacity
    ...
    for i_episode in range(1, 100_000):
        ...
        if episode_count % print_freq == 0:
            print(f"Episode {episode_count} | steps {total_numsteps} "
                  f"| reward {episode_reward:.2f} | critic_loss {c_loss:.4f} "
                  f"| actor_loss {a_loss:.4f} | alpha {alpha_val:.4f}")
            wandb.log({
...
def test(actor_path, critic_path, hidden_size=256, n_episodes=20):
    ...
    test_env = gym.make(env_name, render_mode="human")  # render for visualization
...
if __name__ == '__main__':
    ...
    test("preTrained/sac_actor_LunarLanderContinuous-v3_step400000_ep1229.pth",
         "preTrained/sac_critic_LunarLanderContinuous-v3_step400000_ep1229.pth")
